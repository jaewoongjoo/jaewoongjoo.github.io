...
##### Examples
1. Poisson data : $Y_i \sim poisson(\mu_i)$. Then,
    $$\begin{eqnarray*}
...
2. Binary data : $Y_i \sim Bin(n_i, \pi_i)$. That is
...
1. $Y_i\sim poisson(\mu_i)$ : $\mu_i=e^{\theta_i}$, $b(\theta_i)= e^{\theta_i}$, and $a(\phi)=1$.
...
2. $n_iY_i\sim Bin(n_i, \pi_i)$, where $\theta_i=\log\frac{\pi_i}{1-\pi_i}$, $b(\theta_i)=\log(1+e^{\theta_i})$, $a(\phi)=1/n_i$. 
...
1. $Y_i \sim poisson(\mu_i)$ : $\mu_i=b'(\theta_i)=e^{\theta_i}=g^{-1}(\theta_i)$.
 
      * Canonical link : $\theta_i=g(\mu_i)=\log \mu_i$.
   
   Thus, log link is canonical. 
  
   &nbsp;
   
2. $n_iY_i\sim Bin(n_i,\pi_i)$ : $\pi_i=b'(\theta_i)=\frac{e^{\theta_i}}{1+e^{\theta_i}}=g^{-1}(\theta_i)$.
